...
std::array<u32, PadHandlerBase::button::button_count> PadHandlerBase::get_mapped_key_codes(const std::shared_ptr<PadDevice>& device, const pad_config* profile)
{
	...
	device->trigger_left  = FindKeyCode(button_list, profile->l2);
	device->trigger_right = FindKeyCode(button_list, profile->r2);
	device->axis_left[0]  = FindKeyCode(button_list, profile->ls_left);
	device->axis_left[1]  = FindKeyCode(button_list, profile->ls_right);
	device->axis_left[2]  = FindKeyCode(button_list, profile->ls_down);
	device->axis_left[3]  = FindKeyCode(button_list, profile->ls_up);
	device->axis_right[0] = FindKeyCode(button_list, profile->rs_left);
	device->axis_right[1] = FindKeyCode(button_list, profile->rs_right);
	device->axis_right[2] = FindKeyCode(button_list, profile->rs_down);
	device->axis_right[3] = FindKeyCode(button_list, profile->rs_up);
