...
void pad_settings_dialog::ChangeHandler()
{
	...
	case pad_handler::openxr: m_description = tooltips.gamepad_settings.openxr; break;
#ifdef _WIN32
...
QString pad_settings_dialog::GetLocalizedPadHandler(const QString& original, pad_handler handler)
{
	switch (handler)
	{
		...
		case pad_handler::openxr: return tr("OpenXR");
#ifdef _WIN32
...
QString pad_settings_dialog::GetLocalizedPadName(pad_handler handler, const QString& original, usz index)
{
	switch (handler)
	{
		...
		case pad_handler::openxr: return tr("OpenXR #%0").arg(index);
#ifdef _WIN32
