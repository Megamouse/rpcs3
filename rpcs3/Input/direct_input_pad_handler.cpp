...
direct_input_pad_handler::~direct_input_pad_handler()
{
	...
	{
		auto device = std::static_pointer_cast<DirectInputDevice>(binding.first);
		if (device)
		{
			if (device->device)
...
void direct_input_pad_handler::init_config(pad_config* cfg, const std::string& name)
{
...
{
	auto dev = std::static_pointer_cast<DirectInputDevice>(device);
	if (!dev || !dev->device)
...
pad_preview_values direct_input_pad_handler::get_preview_values(std::unordered_map<u64, u16> data, const std::vector<std::string>& buttons)
{
	pad_preview_values preview_values = { 0, 0, 0, 0, 0, 0 };

	if (buttons.size() == 10)
	{
		preview_values[0] = data[FindKeyCodeByString(button_list, buttons[0])];
		preview_values[1] = data[FindKeyCodeByString(button_list, buttons[1])];
		preview_values[2] = data[FindKeyCodeByString(button_list, buttons[3])] - data[FindKeyCodeByString(button_list, buttons[2])];
		preview_values[3] = data[FindKeyCodeByString(button_list, buttons[5])] - data[FindKeyCodeByString(button_list, buttons[4])];
		preview_values[4] = data[FindKeyCodeByString(button_list, buttons[7])] - data[FindKeyCodeByString(button_list, buttons[6])];
		preview_values[5] = data[FindKeyCodeByString(button_list, buttons[9])] - data[FindKeyCodeByString(button_list, buttons[8])];
	}
...
{
	const auto dev = std::static_pointer_cast<DirectInputDevice>(device);
